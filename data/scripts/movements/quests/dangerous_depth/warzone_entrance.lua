...
function warzoneEntrance.onStepIn(creature, item, position, fromPosition, toPosition)
	...
	if item:getPosition() == Position(33829, 32128, 14) then
		if	player:getStorageValue(Storage.DangerousDepths.Access.TimerWarzoneVI) <= os.time() then
	...
	if item:getPosition() == Position(33777, 32192, 14) then
		if 	player:getStorageValue(Storage.DangerousDepths.Access.TimerWarzoneV) <= os.time() then
	...
	if item:getPosition() == Position(33827, 32172, 14) then
		if 	player:getStorageValue(Storage.DangerousDepths.Access.TimerWarzoneIV) <= os.time() then
